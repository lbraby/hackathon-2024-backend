...
with open("WorldFood/dishes.json", "r", encoding="utf-8") as dishes_file:
    ...
    for dish in dishes:
        ...
        cur.execute("insert into dishes (name, description, uri, image_url, cuisine_type) values (?, ?, ?, ?, ?)", (dish, dish_data["description"], dish_data["uri"], dish_data["image_url"], dish_data["cuisine_type"]))
...
with open("WorldFood/dishes.json", "r", encoding="utf-8") as dishes_file:
    dishes = json.load(dishes_file)
    for dish in dishes:
        cur.execute("select id from dishes where name = ?", (dish, ))
...
# create liked_ingredients table
cur.execute("DROP TABLE IF EXISTS liked_ingredients")
cur.execute("create table liked_ingredients (id INTEGER PRIMARY KEY, username TEXT, ingredient_id INTEGER)")
